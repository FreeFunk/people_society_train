...
public class SocietySchoolCourseNodeQuestionService {
	...
	@Transactional(propagation = Propagation.REQUIRED,rollbackFor=Exception.class)
	public String insertNew(SocietySchoolCourseNodeQuestion societySchoolCourseNodeQuestion) {
		societySchoolCourseNodeQuestion.setCreateTime(new Date());
		nodeQuestionMapper.insert(societySchoolCourseNodeQuestion);
		return "";
	}
	...
	public String insertQuestionAndOption(SocietySchoolCourseNodeQuestion question,String[] optionArr) {
		String resuorceId = question.getOwnerNodeId().split(",")[0];
		String ownerNodeId = question.getOwnerNodeId().split(",")[1];
		SocietyNodeResources societyNodeResources = societyNodeResourcesService.loadById(resuorceId);
		...
		if(node != null){
			...
			question.setOwnerNodeId(societyNodeResources.getId());
			question.setOwnerNodeName(node.getNodeName());
			question.setQuestionScore(1);
			question.setId(Guid.guid());
			String result = "";
			if(result != null && result.equals("")){
				...
				for(int i=0;i<optionArr.length;i++){
					...
					if (nodeOption.getIsRight().equals("1")){
						if(anwserIdStr.equals("")){
							anwserIdStr = nodeOption.getId();
						}else {
							anwserIdStr = anwserIdStr +","+ nodeOption.getId();
						}
					}
				}
			}
			question.setQuestionAnswerId(anwserIdStr);
			insertNew(question);
			//统计该章节下题目的数量
			int questionNum = nodeQuestionMapper.countQuestionNumByNodeId(resuorceId);
			//更新习题数量
			node.setQuestionNum(questionNum);
			societyNodeResources.setQuestionNum(questionNum);
	 ...
	 */
	@Transactional(propagation = Propagation.REQUIRED,rollbackFor=Exception.class)
	public String insertQuestionAndOptionGlobe(SocietySchoolCourseNodeQuestion question,String[] optionArr) {
		...
		if(node != null){
			...
			question.setOwnerCourseName(node.getOwnerCourseName());
			//关联章节池中的章节id
			question.setOwnerNodeId(node.getOwnerNodeResourcesId());
			//关联章节池中的章节名称
			question.setOwnerNodeName(node.getOwnerNodeResourcesName());
			question.setQuestionScore(1);
			String result = insert(question);
			if(result != null && result.equals("")){
				String[] titleArr = new String[]{};
				String[] questionAnswer =new String[]{};
				if(!question.getQuestionType().equals("2")){
		...
		if(result != null && result.equals("")){
			...
			if(optionArr.length == nodeOptionList.size()){//题目类型没有改变
				...
				for(int i=0;i<nodeOptionList.size();i++){
					...
					if (nodeOptionList.get(i).getIsRight().equals("1")){
						if(answerIdStr.equals("")){
							answerIdStr = nodeOptionList.get(i).getId();
						}else {
							answerIdStr = answerIdStr +","+ nodeOptionList.get(i).getId();
						}
					}
				}
				...
				for(int i=0;i<optionArr.length;i++) {
					...
					if (nodeOption.getIsRight().equals("1")){
						if(answerIdStr.equals("")){
							answerIdStr = nodeOption.getId();
						}else {
							answerIdStr = answerIdStr +","+ nodeOption.getId();
						}
					}
				}
			}
		}
		question.setQuestionAnswerId(answerIdStr);
		update(question);
		return "";
	}
	...
	public List<SocietySchoolCourseNodeQuestion> selectByAll() {
		return nodeQuestionMapper.selectByAll();
	}

	public void updateList(List<SocietySchoolCourseNodeQuestion> questionList) {
		nodeQuestionMapper.updateList(questionList);
	}

	public List<SocietySchoolCourseNodeQuestionView> selectByNodeId(String nodeId) {
		return nodeQuestionMapper.selectByNodeId(nodeId);
	}

	public String selectByNodeIdAndQuestionName(String nodeId, String questionName) {
		return nodeQuestionMapper.selectByNodeIdAndQuestionName(nodeId,questionName);
	}

	public void updateByNodeIdList(List<SocietySchoolCourseNodeQuestionView> list) {
		nodeQuestionMapper.updateByNodeIdList(list);
	}
}
