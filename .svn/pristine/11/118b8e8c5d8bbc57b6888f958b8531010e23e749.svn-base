...
public class SocietyStudentService {
	...
	public String insertStuUnique(SocietyStudent stu) {
		...
		stu.setOwnerStudentNum(simpleDateFormat.format(stu.getCreateTime()));
...
//		societyStudentTestPaperQuestionOptionService.updateByStuId(map);
	...
	public Map<String,List<String>> excelImportStuInsert(List<SocietyStudent> studentList) {
		...
		for (SocietyStudent s:studentList){
			...
			s.setPassword(pwd);
			//设置ascii值
			s.setStudentIdCardAscii(idCardInAscii(s.getStudentIdCardNum()));
			//设置昵称
			s.setNickName(s.getStudentName());
	...
	public List<SocietyStudentView> selectByPhoneList(String studentPhoneNum, String schoolId) {
		return societyStudentMapper.selectByPhoneList(studentPhoneNum,schoolId);
	}
	...
	public List<SocietyStudentView> selectByIdCardList(String studentIdCardNum, String schoolId) {
		return societyStudentMapper.selectByIdCardList(studentIdCardNum,schoolId);
	}
	...
	public void updateGlobleAndStu(SocietyStudent student, SocietyStudentUnique societyStudentUnique,
		...
		try {
			...
			System.out.println("idcard："+idCardImage+"，face："+faceImageUrl);
			throw new BusinessException("图片上传失败，请重新认证");
		}
...
//				face = facePrefix + FileUtil.saveBase64Img(base64File,fileForder+facePrefix,true);
	...
	public List<SocietyStudentView> selectVoByClassId(String classId) {
		return societyStudentMapper.selectVoByClassId(classId);
	}

	public List<SocietyStudentView> selectList() {
		return societyStudentMapper.selectAllList();
	}

	public List<SocietyStudentView> selectBySchoolId(String schoolId) {
		return societyStudentMapper.selectBySchoolId(schoolId);
	}

	@Transactional(propagation = Propagation.REQUIRED,rollbackFor=Exception.class)
	public void updateByIdList(List<SocietyStudentView> stuList) {
		societyStudentMapper.updateByIdList(stuList);
	}
}
