...
public class SocietySchoolService {
	...
	public List<Dtree> listSchoolGlobleDtree(String schoolId) {
		List<SocietySchoolView> List = societySchoolMapper.listByObjGloble(schoolId);
		List<Dtree> dtreeList = new ArrayList<>();
		for (SocietySchoolView m:List){
			Dtree dtree = new Dtree();
			dtree.setId(m.getId());
	...
	public List<String> selectByXianQuId(String xianquId) {
		return societySchoolMapper.selectByXianQuId(xianquId);
	}
}
