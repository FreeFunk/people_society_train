...
define(['app','commonService'], function(app){
	return app.controller('realNameCtrl',
	...
	function ($scope,$rootScope,$http,$timeout,appConfig,commonService,$routeParams) {
        ...
        //检查学员实名认证状态
        $scope.checkStuRealNameState = function(){
            var url = "/stu/getUserInfoNotRealName.jsn";
            $rootScope.request(url,{},function(data){
                if(data.success){
                    $scope.studentInfo = data.data;
                    //检查实名认证状态
                    var realNameState = $scope.studentInfo.isRealNameAuth;
                    if(realNameState == '1'){
                        $rootScope.changePage("/main");
                    }else{
                        //身份证号加密
                        var studentIdCardNum = $scope.studentInfo.studentIdCardNum;
                        $scope.studentInfo.studentIdCardNum = studentIdCardNum.substring(0,6)+"******"+studentIdCardNum.substring(14,18);
                    }
                }else{
        ...
        //学员实名认证
        $scope.studentRealName = function(){
            ...
            var url = "/stu/studentRealName.jsn";
