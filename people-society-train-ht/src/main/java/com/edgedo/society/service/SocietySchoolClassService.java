...
public class SocietySchoolClassService {
	...
	public List<SocietySchoolClassView> listPage(SocietySchoolClassQuery societySchoolClassQuery){
		...
		list.forEach(classView->{
			...
			//统计完成培训的人数
			int finishedStuNum = societyStudentAndCourseService.countFinishedClassStuNum(classId);
			//查询班级关联的课程
	...
	@Transactional(propagation = Propagation.REQUIRED,rollbackFor=Exception.class)
	public void classStateChange(String id, String classState) {
		societySchoolClassMapper.classStateChange(id,classState);
	}
	 ...
	 * @return
	 */
	public List<SocietySchoolClass> selectByAllClassInfo(String schoolId) {
		return societySchoolClassMapper.selectByAllClassInfo(schoolId);
	}
}
