...
/*	@Autowired
	...
	@RequestMapping(value = "/listpageSupforcountNew",method = RequestMethod.POST)
	public ModelAndView listpageSupforcountNew(@ModelAttribute SocietySchoolClassQuery query){
		ModelAndView modelAndView = new ModelAndView();
		if(query.getOrderBy() == null || "".equals(query.getOrderBy())){
			query.setOrderBy("CREATE_TIME ASC");
		}
	...
	/**
	 * @Author QiuTianZhu
	 * @Description: 调整班级的上课状态
	 * @Date 2020/10/21 11:30
	 **/
	@RequestMapping(value = "/classStateChange",method = RequestMethod.POST)
	public ModelAndView classStateChange(String id,String classState){
		ModelAndView modelAndView = new ModelAndView();
		service.classStateChange(id,classState);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/selectByAllClassInfo",method = RequestMethod.POST)
	public ModelAndView selectByAllClassInfo(String classId){
		ModelAndView modelAndView = new ModelAndView();
		SocietySchoolClass societySchoolClass = service.loadById(classId);
		List<SocietySchoolClass> list = service.selectByAllClassInfo(societySchoolClass.getOwnerSchoolId());
		return buildResponse(modelAndView,list);
	}


	/**
	 * 根据旧的班级id 修改成当前新的班级id
	 * @return
	 */
	@RequestMapping(value = "/changeClassInfo",method = RequestMethod.POST)
	public ModelAndView changeClassInfo(String classIdOld,String classIdNew,String stuId){
		ModelAndView modelAndView = new ModelAndView();
		//新班级信息
		SocietySchoolClass societySchoolClassNew = service.loadById(classIdNew);
		//班级关联课程
		String courseId = societySchoolClassAndCourseService.selectByClassId(classIdNew);
		//旧得学生班级关联信息
		SocietySchoolClassAndStudentView societySchoolClassAndStudentViewOld =
				societySchoolClassAndStudentService.selectVoByClassAndStudent(classIdOld,stuId);
		societySchoolClassAndStudentViewOld.setClassId(societySchoolClassNew.getId());
		societySchoolClassAndStudentViewOld.setClassName(societySchoolClassNew.getClassName());
		societySchoolClassAndStudentViewOld.setProvinceId(societySchoolClassNew.getProvinceId());
		societySchoolClassAndStudentViewOld.setProvinceName(societySchoolClassNew.getProvinceName());
		societySchoolClassAndStudentViewOld.setCityId(societySchoolClassNew.getCityId());
		societySchoolClassAndStudentViewOld.setCityName(societySchoolClassNew.getCityName());
		societySchoolClassAndStudentViewOld.setXianquId(societySchoolClassNew.getXianquId());
		societySchoolClassAndStudentViewOld.setXianquName(societySchoolClassNew.getXianquName());
		societySchoolClassAndStudentService.update(societySchoolClassAndStudentViewOld);
		//修改学生课程关联表
		SocietyStudentAndCourse societyStudentAndCourseOld =
				societyStudentAndCourseService.selectByStudentIdAndClassId(classIdOld,stuId,societySchoolClassNew.getOwnerSchoolId());
		societyStudentAndCourseOld.setClassId(societySchoolClassNew.getId());
		societyStudentAndCourseOld.setClassName(societySchoolClassNew.getClassName());
		societyStudentAndCourseOld.setProvinceId(societySchoolClassNew.getProvinceId());
		societyStudentAndCourseOld.setProvinceName(societySchoolClassNew.getProvinceName());
		societyStudentAndCourseOld.setCityId(societySchoolClassNew.getCityId());
		societyStudentAndCourseOld.setCityName(societySchoolClassNew.getCityName());
		societyStudentAndCourseOld.setXianquId(societySchoolClassNew.getXianquId());
		societyStudentAndCourseOld.setXianquName(societySchoolClassNew.getXianquName());
		societyStudentAndCourseService.update(societyStudentAndCourseOld);
		//判断是否学完
		if("1".equals(societyStudentAndCourseOld.getLearnIsFinished())){
			societySchoolClassNew.setFinishedPersonNum(societySchoolClassNew.getFinishedPersonNum()+1);
			societySchoolClassNew.setTotalLessons(societySchoolClassNew.getTotalLessons()+1);
			//修改证书班级
			SocietyStudentCertificateView societyStudentCertificateView = societyStudentCertificateService.selectByStuIdAndCourId(stuId,courseId);
			if (societyStudentCertificateView!=null){
				societyStudentCertificateView.setClassId(societySchoolClassNew.getId());
				societyStudentCertificateView.setClassName(societySchoolClassNew.getClassName());
			}
		}else {
			societySchoolClassNew.setNotFinishedPersonNum(societySchoolClassNew.getNotFinishedPersonNum()+1);
			societySchoolClassNew.setTotalLessons(societySchoolClassNew.getTotalLessons()+1);
		}
		return buildResponse(modelAndView);
	}
	...
	@RequestMapping(value = "/schoolClassDataSupCount",method = RequestMethod.POST)
	public ModelAndView schoolClassDataSupCount(@ModelAttribute SocietySchoolClassQuery query){
		ModelAndView modelAndView = new ModelAndView();
		User user = getLoginUser();
		SysUser sysUser = sysUserService.loadById(getLoginUser().getUserId());
		query.getQueryObj().setXianquId(sysUser.getXianquId());
...
//				学生记录list
	...
	public String getClassStudyPro(String classId,Integer classPersonNum){
		...
		if(classPersonNum==0){
			...
			double f1 = new BigDecimal(finishNum*100/classPersonNum).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
			return f1+"%";
		}
	}
...
//		第一页： 学校名字 班级编码	 时间
		...
		societySchoolClassAndStudentViewFisrt.setOwnerSchoolName(societySchool.getSchoolName());
		...
		societySchoolClassAndStudentViewSecond.setOwnerSchoolName(societySchool.getSchoolName());
		...
		List<SocietySchoolClassAndStudentView> list = new ArrayList<>();
		List<SocietyStudentView> studentList = societyStudentService.selectVoByClassId(classId);
		for(SocietyStudentView societyStudentView : studentList){
			SocietySchoolClassAndStudentView societySchoolClassAndStudentView = new SocietySchoolClassAndStudentView();
			societySchoolClassAndStudentView.setStudentName(societyStudentView.getStudentName());
			societySchoolClassAndStudentView.setStudentIdCardNum(societyStudentView.getStudentIdCardNum());
			societySchoolClassAndStudentView.setSex(societyStudentView.getStudentSex());
			societySchoolClassAndStudentView.setStudentPhone(societyStudentView.getStudentPhoneNum());
			societySchoolClassAndStudentView.setStudentType(getPersonType(societyStudentView.getPeopleType()));
			societySchoolClassAndStudentView.setCefiType("无");
			societySchoolClassAndStudentView.setStudyInfo(getStudyInfo(societyStudentView.getId(),societySchoolCourse.getTotalLessons()));
			list.add(societySchoolClassAndStudentView);
		}
	...
	public String getClassProgre(String classId,Integer classActPerson){
		if(classActPerson==0){
			...
			double f1 = new BigDecimal((studyFinsih*100)/classActPerson).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
